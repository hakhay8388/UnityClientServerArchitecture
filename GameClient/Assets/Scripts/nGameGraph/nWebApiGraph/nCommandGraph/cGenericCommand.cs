...
namespace Assets.Scripts.nGameGraph.nWebApiGraph.nCommandGraph
{
    public class cGenericCommand : cBaseCommand
    {
        ...
        public cGenericCommand(cGameGraph _Graph, CommandIDs _Command)
                : base(_Graph, _Command)
        {
            try
            {
                CommandDataClass = Type.GetType("Assets.Scripts.nGameGraph.nWebApiGraph.nCommandGraph.nCommands.n" + CommandID.Name + "Command.c" + CommandID.Name + "CommandData");
                CommandReceiverClass = Type.GetType("Assets.Scripts.nGameGraph.nWebApiGraph.nCommandGraph.nCommands.n" + CommandID.Name + "Command.I" + CommandID.Name + "Receiver");
            }
            catch (Exception _Ex)
            {
            }
