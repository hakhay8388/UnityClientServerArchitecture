...
using Master.Server.nUtils.nJsonConverter;
...
namespace Master.Server.nMasterGraph.nWebApiGraph.nCommandGraph
{
    public class cGenericCommand : cBaseCommand
    {
                ...
                : base(_Graph, _Command)
        {
            try
            {
                CommandDataClass = Type.GetType("Master.Server.nMasterGraph.nWebApiGraph.nCommandGraph.nCommands.n" + CommandID.Name + "Command.c" + CommandID.Name + "CommandData");
                CommandReceiverClass = Type.GetType("Master.Server.nMasterGraph.nWebApiGraph.nCommandGraph.nCommands.n" + CommandID.Name + "Command.I" + CommandID.Name + "Receiver");
            }
            catch (Exception _Ex)
            {
            }
        ...
        public override void Interpret(cSession _Session, JToken _JToken)
        {
            try
            {
                ...
                if (!__Event.IsPropogationStoped)
                {
                    for (int i = 0; i < ReceiverList.Count; i++)
                    {
                        ...
                        try
                        {
                            MethodInfo __Receiver = CommandReceiverClass.GetMethod("Receive" + CommandID.Name + "Data", 0, new Type[] { typeof(cSession), typeof(cListenerEvent), __Data.GetType() });
                            __Receiver.Invoke(__ReceiverObject, new object[] { _Session,  __Event, __Data });
                            if (__Event.IsPropogationStoped)
                            {
                                break;
                            }
